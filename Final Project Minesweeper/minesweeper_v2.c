...
void destaparAlrrededores (int i, int j, tipocasilla** matriz,int filas,int columnas){ //Función para abrir las casillas laterales cuando la casilla que se escribe es no tiene minas alrededor.
...
void orden(tipocasilla** matriz,int filas,int columnas){  //Función con la que el jugador pierde y se cambian las minas y el estado para destaparlas cuando se dibuje.
...
int evaluar(int filas,int columnas,tipocasilla** matriz,int num,struct timeval ti,struct timeval tf,int juefi,int jueco){ //Función con la que gana
	double tiempo;
...
void jugar (int filas,int columnas, tipocasilla** matriz,int juefi,int jueco, int num,struct timeval ti,struct timeval tf){ //Funcion con la que el jugador juega en el cmd
	...
	while(can>=0){ //Mientras no pierda (número negativo) que siga preguntando donde quiere poner la fila y la columna
		...
		if ((np==1)&&(juefi>0 && juefi <filas && jueco>0 && jueco<columnas)&& (matriz[juefi][jueco].tipo!='m')){ //La opcion 1 es para destapar casillas, esta condicion dice que no sea una mina para que pueda seguir jugando.
			...
			destaparAlrrededores(juefi,jueco,matriz,filas,columnas);
		...
		can=evaluar(filas,columnas,matriz,num,ti,tf,juefi,jueco); //Se evalua siempre y si retorna -1 significa que ganó y sale del while y acaba el juego
		if (np==3){
...
void main(int argc, char* argv[]){
	...
	if (lvl==4){ //Opcion cuatro (Personalizado)
		...
		scanf("%d",&filas);
		filas=filas+2;
		printf("Escriba el número de columnas: ");
		scanf("%d",&columnas);
		columnas=columnas+2;
		printf("Escriba un número de minas: ");
		scanf("%d",&num);
	}
	...
	destaparAlrrededores(filin,colin,tablero,filas,columnas);
